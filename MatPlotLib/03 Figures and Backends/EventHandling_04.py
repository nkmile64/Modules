...
class DraggableRectangle:
    ...
    def on_press(self, event: MouseEvent):
        ...
        if event.inaxes != self.rect.axes or DraggableRectangle.lock is not None:
            return
        ...
        axes = self.rect.axes
        # the rectangle is intended to be used in an animation,
        # so exclude it from regular drawing of the figure.
        ...
        # and blit just the redraw area
        canvas.blit(axes.bbox)

    def on_motion(self, event: MouseEvent):
        """Move the rectangle if the mouse is over us"""
        if event.inaxes != self.rect.axes or DraggableRectangle.lock is not self:
            return
